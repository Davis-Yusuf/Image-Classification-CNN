...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        feat_list = []
        for i in feat:
            feat_list.append(i.flatten())
        output = np.array(feat_list)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        for i in range(len(feat.shape)):
            shape_size.append(feat.shape[i])
            
        shape_size = tuple(shape_size)     
        dfeat = np.reshape(dprev, (shape_size))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class gelu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = 0.5*np.tanh(0.0356774*(feat**3)+0.797885*feat)+0.5+(0.0535161*(feat**3) + 0.398942*feat)*(np.cosh(0.0356774*(feat**3)+0.797885*feat))**(-2)
        dfeat = dprev * dfeat
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training:
            if self.keep_prob == 0 or self.keep_prob == 1:
                ...
                rand_nums = self.rng.rand(*feat.shape)
                kept = rand_nums < self.keep_prob
                temp_zero = np.ones((feat.shape[0], feat.shape[1]))
                new_feat = temp_zero*kept
                new_feat = new_feat*feat
                output = new_feat / self.keep_prob
        else:
            output = feat
        #############################################################################
        #                             END OF YOUR CODE                              #
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        N = feat.shape[0]
        one_hot_vec = np.eye(feat.shape[1])[label]
        loss = -1/N * np.sum(one_hot_vec * np.log(logit))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
    ...
    def backward(self):
        ...
        #############################################################################
        N = label.shape[0]
        one_hot_vec = np.eye(logit.shape[1])[label]
        dlogit = (logit - one_hot_vec) / N
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
...
def softmax(feat):
    ...
    #############################################################################
    exponentials = np.exp(feat - np.max(feat, axis=1, keepdims=True))
    scores = exponentials / np.sum(exponentials, axis=1, keepdims=True)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
