...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
    ...
    for epoch in range(max_epochs):
        ...
        for iter in tqdm(range(iter_start, iter_end)):
            ...
            #############################################################################
            forward_out = model.forward(data_batch)
            loss_forward = loss_func.forward(forward_out, labels_batch)
            loss_hist.append(loss_forward)
            
            loss_backward = loss_func.backward()
            backward_out = model.backward(loss_backward, regularization=regularization, reg_lambda=reg_lambda)
            
            optimizer.update(model.net)       
            #############################################################################
            #                             END OF YOUR CODE                              #
            #############################################################################
