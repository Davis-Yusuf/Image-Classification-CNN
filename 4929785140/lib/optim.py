...
class SGD(Optimizer):
    ...
    def update(self, layer):
        for n, dv in layer.grads.items():
            ...
            #############################################################################
            old_weights = layer.params[n]
            new_weights = old_weights - (self.lr * dv) - (self.weight_decay * old_weights)
            self.net.assign(n, new_weights)
            #############################################################################
            #                             END OF YOUR CODE                              #
            #############################################################################
    ...
    def update(self, layer):
        ...
        for pname, pval in layer.params.items():
            ...
            if self.weight_decay != 0.0:
                g += self.weight_decay * pval
            if pname not in self.mt:
                self.mt[pname] = np.zeros_like(g)
                self.vt[pname] = np.zeros_like(g)
            self.mt[pname] = self.beta1 * self.mt[pname] + (1 - self.beta1) * g
            self.vt[pname] = self.beta2 * self.vt[pname] + (1 - self.beta2) * (g ** 2)
            mthat = self.mt[pname] / (1 - self.beta1 ** self.t)
            vthat = self.vt[pname] / (1 - self.beta2 ** self.t)
            pval -= self.lr * mthat / (np.sqrt(vthat) + self.eps)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
